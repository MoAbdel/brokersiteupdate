...
export default function ClosingCostsCalculatorClient() {
  ...
  const calculateClosingCosts = () => {
    ...
    const originationFee = loan * 0.008;
    ...
    const transferTax = price * 0.001;
    const governmentFees = recordingFees + transferTax;
    const homeInsurance = (price * 0.004) / 12;
    const propertyTaxes = (price * 0.0075) / 12;
    const prepaidInterest = (loan * 0.065) / 365 * 15;
    const prepaidExpenses = homeInsurance + propertyTaxes + prepaidInterest;
    const escrowInsurance = homeInsurance * 2;
    const escrowTaxes = propertyTaxes * 2;
    const escrowDeposit = escrowInsurance + escrowTaxes;

    let pmiPremium = 0;
    if (loanType === 'conventional' && (loan / price) > 0.8) {
      pmiPremium = loan * 0.005 / 12;
    }
    ...
    const totalCashNeeded = totalClosingCosts + (price - loan);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Home Purchase Price</label>
              ...
              <Button onClick={calculateClosingCosts} className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3 text-lg font-semibold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex justify-between"><span className="text-slate-600">Origination Fee:</span><span>{formatCurrency(results.breakdown.originationFee)}</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Underwriting:</span><span>$500</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Processing:</span><span>$400</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Credit Report:</span><span>$50</span></li>
                </ul>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-slate-800 mb-3">Third-Party</h3>
                <ul className="space-y-2 text-sm">
                  <li className="flex justify-between"><span className="text-slate-600">Title Insurance:</span><span>{formatCurrency(results.breakdown.titleInsurance)}</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Appraisal:</span><span>$600</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Inspection:</span><span>$500</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Attorney:</span><span>$800</span></li>
                </ul>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-slate-800 mb-3">Prepaid/Escrow</h3>
                <ul className="space-y-2 text-sm">
                  <li className="flex justify-between"><span className="text-slate-600">Home Insurance:</span><span>{formatCurrency(results.breakdown.homeInsurance)}</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Property Taxes:</span><span>{formatCurrency(results.breakdown.propertyTaxes)}</span></li>
                  <li className="flex justify-between"><span className="text-slate-600">Prepaid Interest:</span><span>Varies</span></li>
                </ul>
              </div>
            </div>

            <div className="mt-8 pt-8 border-t border-slate-200 text-center">
              <p className="text-slate-600 mb-4">Get a personalized closing cost estimate and explore ways to reduce costs.</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="tel:(949) 579-2057">
                  <Button className="bg-slate-900 hover:bg-slate-800 text-white px-8 py-3">Call (949) 579-2057</Button>
                </a>
                <a href="/contact">
                  <Button variant="ghost" className="border border-slate-300 px-8 py-3">Get Detailed Estimate</Button>
                </a>
              </div>
            </div>
          </div>
