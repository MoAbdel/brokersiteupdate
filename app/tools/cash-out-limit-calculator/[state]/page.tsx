...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return genMeta({
    title: `Cash-Out Refinance Calculator — All ${counties.length} ${stateName} Counties [2026 FHFA Limits]`,
...
export default async function StatePage({ params }: Props) {
  ...
  // Sort by median home price descending for discovery
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1 text-sm text-slate-500">
            ...
            <li><span className="mx-1">/</span></li>
            <li><Link href="/tools/cash-out-limit-calculator" className="hover:text-blue-600">Cash-Out Calculator</Link></li>
            <li><span className="mx-1">/</span></li>
        ...
        <div className="text-center mb-10">
          <Badge className="bg-emerald-100 text-emerald-700 mb-4">{counties.length} Counties</Badge>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mb-4">
            {stateName} Cash-Out Refinance Calculator by County
          </h1>
          <p className="text-lg text-slate-600 max-w-3xl mx-auto">
            {stateName} has {counties.length} counties with an average conforming loan limit of{' '}
            {formatCurrency(avgLimit)} for 2026. Select your county for a personalized cash-out analysis.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {sorted.map((county) => (
            <Link
              key={county.slug}
              href={`/tools/cash-out-limit-calculator/${state}/${county.slug}`}
              className="group"
            >
