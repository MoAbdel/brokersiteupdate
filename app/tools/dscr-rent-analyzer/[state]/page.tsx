...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return genMeta({
    title: `DSCR Rent Analyzer — All ${counties.length} ${stateName} Counties [2026 Investment Property Cash-Flow Tool]`,
...
export default async function StatePage({ params }: Props) {
  ...
  const sorted = [...counties].sort((a, b) => b.medianRent - a.medianRent);
  const avgRent = Math.round(counties.reduce((s, c) => s + c.medianRent, 0) / counties.length);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1 text-sm text-slate-500">
            ...
            <li><span className="mx-1">/</span></li>
            <li><Link href="/tools/dscr-rent-analyzer" className="hover:text-blue-600">DSCR Rent Analyzer</Link></li>
            <li><span className="mx-1">/</span></li>
        ...
        <div className="text-center mb-10">
          <Badge className="bg-purple-100 text-purple-700 mb-4">{counties.length} Counties</Badge>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mb-4">
            {stateName} DSCR Rent Analyzer by County
          </h1>
          <p className="text-lg text-slate-600 max-w-3xl mx-auto">
            {stateName} has {counties.length} counties with an average median rent of{' '}
            {formatCurrency(avgRent)}/mo. Select your county for a personalized DSCR cash-flow analysis.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {sorted.map((county) => (
            <Link
              key={county.slug}
              href={`/tools/dscr-rent-analyzer/${state}/${county.slug}`}
              className="group"
            >
              <Card className="hover:shadow-md hover:border-purple-300 transition-all h-full">
                <CardContent className="p-5">
                  <h2 className="font-semibold text-slate-900 group-hover:text-purple-600 transition-colors mb-2">
                    {county.name}
                  </h2>
                  <dl className="grid grid-cols-2 gap-2 text-xs text-slate-500">
                    <div>
                      <dt>Median Rent</dt>
                      <dd className="font-semibold text-slate-700">{formatCurrency(county.medianRent)}/mo</dd>
                    </div>
                    <div>
                      <dt>Median Home</dt>
