  ...
  return (
    <>
      ...
      <button
        ...
        className="hidden sm:flex fixed left-4 z-[99995] items-center justify-center w-10 h-10 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg shadow-blue-600/40 transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2"
          ...
          <div className="p-3 flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2">
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() =>
                    update({
                      fontSizeStep: Math.max(-2, prefs.fontSizeStep - 1),
                    })
                  }
                ...
                <button
                  onClick={() =>
                    update({
                      fontSizeStep: Math.min(3, prefs.fontSizeStep + 1),
                    })
                  }
            ...
            {(
              [
                {
                  key: "highContrast",
                  label: "🌙 High Contrast",
                  val: prefs.highContrast,
                },
                {
                  key: "dyslexiaFont",
                  label: "🔤 Dyslexia Font",
                  val: prefs.dyslexiaFont,
                },
                {
                  key: "highlightLinks",
                  label: "🔗 Highlight Links",
                  val: prefs.highlightLinks,
                },
                {
                  key: "reduceMotion",
                  label: "🚫 Reduce Motion",
                  val: prefs.reduceMotion,
                },
                {
                  key: "readingGuide",
                  label: "📖 Reading Guide",
                  val: prefs.readingGuide,
                },
              ] as { key: keyof A11yPrefs; label: string; val: boolean }[]
            ).map(({ key, label, val }) => (
              <div
                key={key}
              ...
              >
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${val ? "translate-x-5" : "translate-x-0"}`}
                  />
                </button>
              </div>
            ))}
