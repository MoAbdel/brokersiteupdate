# Geo-Targeted Content Templates — Governance Model (2026)

## Architecture Change: Generic City Pages → Hub-First, With Approved Refinance-City Exceptions

**Previous model:** One page per city (3,250-4,000 words each)
**Current default:** Hub/support content grouping related cities or intents
**Approved exception:** 16 refinance city conversion pages using `components/seo/RefinanceCityTemplate.tsx`

**Reason for change:** 32 individual city pages were marked "Crawled - not indexed" by Google in Feb 2026 due to near-duplicate content. The governance model solves this by:
...
- Allowing standalone city pages only when the funnel owner is intentionally the refinance-city page
...
**Note:** These templates are retained for reference and for refreshing already-strong support pages only. New generic city content should use the hub/support model above.

**Exception:** The approved 16 refinance city pages are not generated from these legacy templates. They must use `components/seo/RefinanceCityTemplate.tsx`.
